<x-admin-layout :title="'Edit Amenity'">
    ...
    <div class="rounded-2xl border bg-white p-5 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                <div class="text-lg font-semibold">Edit Amenity</div>
                <div class="text-sm text-slate-600">{{ $amenity->name }}</div>
            </div>
        ...
        <form class="mt-6" method="POST" action="{{ route('admin.amenities.update', $amenity) }}">
            @csrf
            @method('PUT')
            @include('admin.amenities.form', ['amenity' => $amenity])
