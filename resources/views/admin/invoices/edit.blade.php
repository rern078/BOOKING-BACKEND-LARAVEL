<x-admin-layout :title="'Edit Invoice'">
    @include('admin.partials.flash')

    <div class="rounded-2xl border bg-white p-5 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                <div class="text-lg font-semibold">Edit Invoice</div>
                <div class="text-sm text-slate-600">{{ $invoice->number }}</div>
            </div>
        ...
        <form class="mt-6" method="POST" action="{{ route('admin.invoices.update', $invoice) }}">
            @csrf
            @method('PUT')
